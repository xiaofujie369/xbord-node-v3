...
class ClientController extends Controller
{
    ...
    public function doSubscribe(Request $request, $user, $servers = null)
    {
        ...
        // REALITY subscription support is a separate capability. Do not turn an
        // AnyTLS REALITY/plain node into a misleading certificate-TLS subscription.
        $serversFiltered = array_values(array_filter($serversFiltered, fn ($server) =>
            $server['type'] !== Server::TYPE_ANYTLS
                || is_array(data_get($server, 'protocol_settings.tls'))
                || (int) data_get($server, 'protocol_settings.tls', 1) === 1
        ));
        $serversFiltered = array_map(function ($server) {
            unset($server['protocol_settings']['reality_settings']['private_key']);
            if ($server['type'] === Server::TYPE_ANYTLS && !is_array(data_get($server, 'protocol_settings.tls'))) {
                // Keep existing standard-TLS formatters compatible with the old
                // object-valued tls shape while storage/node APIs use a mode.
                $server['protocol_settings']['tls'] = data_get($server, 'protocol_settings.tls_settings', []);
            }
            return $server;
        }, $serversFiltered);
