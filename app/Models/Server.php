...
class Server extends Model
{
    ...
    public function getProtocolSettingsAttribute($value)
    {
        ...
        if ($this->type === self::TYPE_ANYTLS) {
            $settings = self::normalizeAnyTLSSettings($settings);
        }
    ...
    public function setProtocolSettingsAttribute($value)
    {
        ...
        if ($this->type === self::TYPE_ANYTLS) {
            $value = self::normalizeAnyTLSSettings($value ?? []);
        }
    ...
    /** Read old rows and accept old admin clients without a database migration. */
    public static function normalizeAnyTLSSettings(array $settings): array
    {
        if (is_array($settings['tls'] ?? null)) {
            $settings['tls_settings'] = $settings['tls'];
            $settings['tls'] = 1;
        }
        return $settings;
    }
