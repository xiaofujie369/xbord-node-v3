...
class ServerSave extends FormRequest
{
    ...
    private function getProtocolRules(string $type): array
    {
        ...
        return match ($type) {
            ...
            'anytls' => array_merge(
                ...
                self::REALITY_RULES,
                [
                    'reality_settings.server_name' => 'required_if:protocol_settings.tls,2|nullable|string',
                    'reality_settings.dest' => 'required_if:protocol_settings.tls,2|nullable|string',
                    'reality_settings.private_key' => 'required_if:protocol_settings.tls,2|nullable|string|regex:/^[A-Za-z0-9_-]{43}$/',
                    'reality_settings.public_key' => 'required_if:protocol_settings.tls,2|nullable|string|regex:/^[A-Za-z0-9_-]{43}$/',
                    'reality_settings.short_id' => 'nullable|string|regex:/^(?:[a-fA-F0-9]{2}){0,8}$/',
                ],
            ),
            default => $rules,
        };
    ...
    {
        if ($this->input('type') === Server::TYPE_ANYTLS && is_array($this->input('protocol_settings'))) {
            $this->merge(['protocol_settings' => Server::normalizeAnyTLSSettings($this->input('protocol_settings'))]);
        }
    }
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            if ($this->input('type') !== Server::TYPE_ANYTLS || (int) $this->input('protocol_settings.tls') !== 2) {
                return;
            }
            $prefix = 'protocol_settings.reality_settings.';
            $dest = $this->input($prefix . 'dest');
            // Match the existing node builder's host:port contract (no IPv6 literals).
            if (is_string($dest) && (!preg_match('/^[^:\s\/]+:([0-9]+)$/D', $dest, $matches)
                || (int) $matches[1] < 1 || (int) $matches[1] > 65535)) {
                $validator->errors()->add($prefix . 'dest', 'REALITY destination must be host:port with port 1–65535.');
            }
            $serverName = $this->input($prefix . 'server_name');
            if (is_string($serverName) && preg_match('/[\s:\/]/', $serverName)) {
                $validator->errors()->add($prefix . 'server_name', 'REALITY server name must be a hostname.');
            }
            $private = $this->input($prefix . 'private_key');
            $public = $this->input($prefix . 'public_key');
            if (!is_string($private) || !is_string($public)
                || !preg_match('/^[A-Za-z0-9_-]{43}$/D', $private)
                || !preg_match('/^[A-Za-z0-9_-]{43}$/D', $public)) {
                return;
            }
            $privateBytes = base64_decode(strtr($private, '-_', '+/') . '=', true);
            $expected = rtrim(strtr(base64_encode(sodium_crypto_scalarmult_base($privateBytes)), '+/', '-_'), '=');
            if (!hash_equals($expected, $public)) {
                $validator->errors()->add($prefix . 'public_key', 'REALITY public key does not match the private key.');
            }
        });
    }
